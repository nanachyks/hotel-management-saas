...
employeesRouter.get('/', async (req: AuthRequest, res: Response) => {
  ...
  const employees = await db.queryAll(query, params);
  res.json(employees);
});

employeesRouter.post('/', async (req: AuthRequest, res: Response) => {
  ...
  const hotelId = req.user?.hotel_id;
  if (department_id) {
    const dept = await db.queryOne('SELECT id FROM departments WHERE id = ? AND hotel_id = ?', [department_id, String(hotelId)]);
  ...
  const created = await db.queryOne('SELECT e.*, d.name as department_name FROM employees e LEFT JOIN departments d ON e.department_id = d.id WHERE e.id = ?', [id]);
  res.status(201).json(created);
});

employeesRouter.put('/:id', async (req: AuthRequest, res: Response) => {
  const hotelId = req.user?.hotel_id;
  const existing = await db.queryOne('SELECT * FROM employees WHERE id = ? AND hotel_id = ?', [req.params.id, String(hotelId)]);
  ...
  const updated = await db.queryOne('SELECT e.*, d.name as department_name FROM employees e LEFT JOIN departments d ON e.department_id = d.id WHERE e.id = ?', [req.params.id]);
  res.json(updated);
});

employeesRouter.delete('/:id', async (req: AuthRequest, res: Response) => {
  const hotelId = req.user?.hotel_id;
  const existing = await db.queryOne('SELECT * FROM employees WHERE id = ? AND hotel_id = ?', [req.params.id, String(hotelId)]);
  if (!existing) return res.status(404).json({ error: 'Employee not found' });
  await db.execute('DELETE FROM employees WHERE id = ? AND hotel_id = ?', [req.params.id, String(hotelId)]);
