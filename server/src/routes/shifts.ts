...
shiftsRouter.get('/', (req: AuthRequest, res: Response) => {
  ...
  const shifts = db.queryAll(query, params);
  res.json(shifts);
});

shiftsRouter.post('/', (req: AuthRequest, res: Response) => {
  ...
  const created = db.queryOne(`SELECT s.*, e.first_name || ' ' || e.last_name as employee_name FROM shifts s JOIN employees e ON s.employee_id = e.id WHERE s.id = ?`, [id]);
  res.status(201).json(created);
});

shiftsRouter.put('/:id', (req: AuthRequest, res: Response) => {
  const hotelId = req.user?.hotel_id;
  const existing = db.queryOne('SELECT * FROM shifts WHERE id = ? AND hotel_id = ?', [req.params.id, String(hotelId)]);
  if (!existing) return res.status(404).json({ error: 'Shift not found' });
  const { employee_id, date, start_time, end_time, notes } = req.body;
  db.execute('UPDATE shifts SET employee_id = ?, date = ?, start_time = ?, end_time = ?, notes = ? WHERE id = ? AND hotel_id = ?',
  ...
  const updated = db.queryOne(`SELECT s.*, e.first_name || ' ' || e.last_name as employee_name FROM shifts s JOIN employees e ON s.employee_id = e.id WHERE s.id = ?`, [req.params.id]);
  res.json(updated);
});

shiftsRouter.delete('/:id', (req: AuthRequest, res: Response) => {
  const hotelId = req.user?.hotel_id;
  const existing = db.queryOne('SELECT * FROM shifts WHERE id = ? AND hotel_id = ?', [req.params.id, String(hotelId)]);
  if (!existing) return res.status(404).json({ error: 'Shift not found' });
  db.execute('DELETE FROM shifts WHERE id = ? AND hotel_id = ?', [req.params.id, String(hotelId)]);
