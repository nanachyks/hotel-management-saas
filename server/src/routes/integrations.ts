...
integrationsRouter.get('/providers/:type', async (req: AuthRequest, res: Response) => {
  const providers = PROVIDERS[req.params.type] || [];
  const connectedRows = await db.queryAll(
    'SELECT provider FROM integrations WHERE hotel_id = ? AND type = ?',
    [req.user!.hotel_id, req.params.type]
  );
  const connected = connectedRows.map((r: any) => r.provider);
...
integrationsRouter.post('/', async (req: AuthRequest, res: Response) => {
  ...
  const existing = await db.queryOne('SELECT id FROM integrations WHERE hotel_id = ? AND type = ? AND provider = ?', [req.user!.hotel_id, type, provider]);
  ...
  res.status(201).json(await db.queryOne('SELECT * FROM integrations WHERE id = ?', [id]));
});

integrationsRouter.put('/:id', async (req: AuthRequest, res: Response) => {
  const existing = await db.queryOne('SELECT * FROM integrations WHERE id = ? AND hotel_id = ?', [req.params.id, req.user!.hotel_id]);
  ...
  await db.execute(
    ...
    [name ?? existing.name, api_key ?? existing.api_key, api_secret ?? existing.api_secret,
     ...
     enabled !== undefined ? !!enabled : existing.enabled, req.params.id, req.user!.hotel_id]
  );
  res.json(await db.queryOne('SELECT * FROM integrations WHERE id = ?', [req.params.id]));
});

integrationsRouter.delete('/:id', async (req: AuthRequest, res: Response) => {
  const existing = await db.queryOne('SELECT * FROM integrations WHERE id = ? AND hotel_id = ?', [req.params.id, req.user!.hotel_id]);
  if (!existing) return res.status(404).json({ error: 'Integration not found' });
  await db.execute('DELETE FROM integrations WHERE id = ?', [req.params.id]);
  res.json({ message: 'Integration removed' });
});

integrationsRouter.post('/:id/sync', async (req: AuthRequest, res: Response) => {
  const integration = await db.queryOne('SELECT * FROM integrations WHERE id = ? AND hotel_id = ?', [req.params.id, req.user!.hotel_id]);
  ...
  if (integration.type === 'channel_manager') {
    ...
    const recentPayments = await db.queryAll("SELECT COUNT(*) as count, COALESCE(SUM(amount),0) as total FROM payments WHERE created_at >= NOW() - INTERVAL '30 days'", []);
    result.recentPayments = recentPayments[0];
  } else if (integration.type === 'accounting') {
    const invoices = await db.queryAll("SELECT COUNT(*) as count, COALESCE(SUM(amount),0) as total FROM invoices WHERE hotel_id = ? AND status IN ('paid','partial')", [req.user!.hotel_id]);
    result.exportReady = invoices[0];
  } else if (integration.type === 'pos') {
    const services = await db.queryAll('SELECT COUNT(*) as count FROM services WHERE hotel_id = ?', [req.user!.hotel_id]);
    result.menuItems = services[0]?.count || 0;
  } else if (integration.type === 'door_lock' || integration.type === 'key_card') {
    const rooms = await db.queryAll('SELECT COUNT(*) as count FROM rooms WHERE hotel_id = ?', [req.user!.hotel_id]);
    result.rooms = rooms[0]?.count || 0;
  }

  await db.execute("UPDATE integrations SET last_sync_at = NOW() WHERE id = ?", [req.params.id]);
  res.json(result);
});
